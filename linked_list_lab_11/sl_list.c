...
intnode_t * add(intnode_t * head, int elem, int index)
{
	...
	int list_size = length(head);
	assert(index <= list_size);

	int i = 0;
	intnode_t * tempnode = intnode_construct(elem, NULL);
	intnode_t * current;

	if (index == 0)	//if before first element
	{
		head = push(head, elem);
	}
...
// Functions added from previous lab
